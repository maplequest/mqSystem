...
class mqColorFilter {
  constructor(target, baseColor) {
    ...
    this.reusedColor = new mqColor(0, 0, 0);
  }
  solve() {
...
* mqColorToRGB('#fff');
* > [255,255,255]
* mqColorToRGB('var(--mqDanger)');
* > [229,62,62]
*/
function mqColorToRGB(color) {
  var o = mqMakeWidget('div','color-expander');
  mqStyle(o,'color',color);
  mqStyle(o,'display','none');
  //mqAppend('mq-top',o);
  mqAppend(document.body,o);
  var res = mqStyle(o,'color');
  mqDelete(o);
  if (res.slice(3,4)=='a') {
...
*/
function mqColorToFilter (c) {
  const rgb = mqColorToRGB(c);
  const color = new mqColor(rgb[0], rgb[1], rgb[2]);
  const filter = new mqColorFilter(color);
